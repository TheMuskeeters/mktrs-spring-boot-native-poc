...
public record UserController(UserService userService) {
    ...
    /**
     * Retrieve one users registered in the system.
     * <p>{@code GET: api/v1/users/{userId} }</p>
     *
     * @param userId Indicates the users unique identifier to search. If it is empty or NULL an exception is thrown.
     ...
     *
     * @param user Includes the users information to insert.
    ...
    public User insertUser(@Valid @RequestBody User user) {
        ...
        log.info("==> Payload users=[" + user + "]");

        return userService.insert(user);
    }

    /**
     * Modifies the data for the users.
     * <p>{@code PATCH: api/v1/users}</p>
     * <p>For the User to be inserted there are validations for required fields, {@code name} and {@code address}.
     * A BAD REQUEST 400 error code is returned when {@code payload} is mal formed.</p>
     *
     * @param user Includes the users information to update.
     ...
     *
     * @param userId Indicates the users unique identifier to search. If it is empty or NULL an exception is thrown.
     * @return HTTP 200 if removed, HTTP 404 if users record not found.
     */
